...
public class FilterImpl extends AbstractPersistable<FilterPojo> implements Filter {
  ...
  protected FilterPojo writeTo(FilterPojo pojo) {
    ...
    return pojo.setDescription(description).setSuspended(isSuspended).setPriority(priority);
  }
  ...
  private void readFromCurrentOrFutureVersion(FilterPojo pojo) {
    ...
    this.priority = readPriority(pojo);
  }

  private byte readPriority(FilterPojo pojo) {
    byte priority = pojo.getPriority();
    if (priority < 1) {
      return 1;
    } else if (priority > 10) {
      return 10;
